...
- インターフェースのみ提供し、実装の詳細（データのこと）を隠蔽できることで、変更により影響が及ぶ範囲が小さくなる
- ひと目でわかる
...
- カプセル化＝外部からの、メンバルーチンへのアクセスは最小限にし、メンバデータは公開しないこと
- カプセル化の「意味的な違反」に注意する
...
- 重複を排除する
- ルーチンを十分に分解し短く保つことで、サブクラスの作成を楽にする
- ポインタ関連処理を囲いだし、隠蔽する
- 移植性のない機能を囲だし、移植性を向上させる
- 複雑な論理評価を囲いだし、単純にする
- 1 箇所のコードの改善が、全体に波及する
...
ルーチンが決まった順序で実行する処理で構成される場合。生年月日（入力データ）から年齢を計算し、年齢をもとに定年までの期間を計算する、など。
...
引数のフラグにより処理を分岐させる場合。分岐させるという目的のためだけに機能がまとめられている場合。
...
- 戻り値を表す
  ...
  - `customerId.Next()`
  - `pen.CurrentColor()` など
- 機能的凝集度であれば、オブジェクトを操作することが多いので、動詞＋オブジェクト名の形にする
  - `PrintDocument()`
  - `checkOrderInfo()`など
