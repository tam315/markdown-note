...
`while-break`を使うと、ループの最初や最後ではなく **途中** に出口があるループを作ることができる。下記のように、重複処理を避けるために使うとよい。
...
- 単純な処理にのみ使用する
- for を途中で抜けたいなど、複雑なループには`while`を使用する
- for を途中で抜けたいがために、ループ変数を変更してはならない
...
- 安全カウンタ（上限）を適宜使用する
- while ループでは、フラグよりも break を使うときれいになりやすい
- 複数の break には要注意
