...
- TDD のメリット
  - すべきことは何かを事前に整理せざるを得ない
  - 実装よりもインターフェースに集中せざるを得ない
  - コーディングの完了時点が明白
- フィクスチャ
  - テストに必要となるデータやオブジェクトのこと
- `test`文に与える説明文は、失敗したときにどのテストか識別できる程度に書くとよい。
  - 説明しすぎは冗長なコメントのようなもの
- どこに、どれくらいテストを書くべきか
  ...
  - 以下を集中的にテストすること
    ...
    - 複雑なところ
    - エラーが起こりそうなところ
  - 例えばセッターをテストするなどは明らかに無意味
- fixture をテスト間で共有するな。大変なトラブルのもとになるから。

```ts
// Bad
describe('', () => {
  const asia = new Province();
  // many test cases goes here..
});
...
- fixture を共通した方法でセットアップし、テスト文で少し改変したうえで検証するのは、よくあるやり方
  - いろんな呼び名がついている
