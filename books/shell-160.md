...
- 標準出力先をファイルに変更（リダイレクト）するには`>`or`1>`記号を使う
...
- 標準入力元をファイルに変更（リダイレクト）するには`<`or`0<`記号を使う
...
- 標準エラー出力先を変更（リダイレクト）するには
  - `2>&1` - stderrの出力先を、いまstdoutがつながっている先に振り向ける
  - `|&` - stdout, stderrをどちらもパイプする

```sh
# これだと標準エラー出力は画面に渡され、コマンドに渡されないので
sed '////' | wc
# こうする
sed '////' 2>&1 | wc
sed '////' |& wc
