...
変数に値を代入しようが、ポインタに値を代入しようが、本質は同じ。どちらも、メモリ上に`123`などの具体的な値を格納したうえで、その **メモリアドレス** を保持していると考えられる。

本質的な違いは、値へのアクセス方法と、代入時等の振る舞いである。

- 値へのアクセス方法
  - 変数なら`i`で値, `&i`でアドレス
  - ポインタなら`p`でアドレス、`*p`で値
- 代入時等の振る舞い
  - 変数なら、代入等したときに「値」がコピーされる
  - ポインタなら、代入等したときに「アドレス」がコピーされる
